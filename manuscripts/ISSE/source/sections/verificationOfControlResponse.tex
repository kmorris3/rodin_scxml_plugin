...
\begin{theorem}[System Events are Relative Deadlock-Free]
...
\begin{proof}
  ...
  \item When the system is in the \emph{Ready to De-queue} state, %
    |uc = TRUE & dt = {}|.  %
    In the case where |iQ /= {}|, the |dequeueInternalTrigger| event
    is enabled. In the case where |iQ = {}|, according to our
    assumption, we have |eQ /= {}|, hence |dequeueExternalTrigger|
    event is enabled.
...
\begin{assumption}[Strongly Fair System Events]
  We assume that the systems event that is |dequeueExternalTrigger|
  and \emph{internal events} are strongly fair, i.e., if a system event is
  enabled infinitely often, eventually, they will be taken.
\end{assumption}

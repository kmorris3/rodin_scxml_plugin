...
\SonComment{Make explicit definition of strong fairness and its interaction with properties}
...
\KarlaComment{SON: Could you please take a look just to make 
sure I did not introduced mistakes with my changes}
\label{sec:proof-convergence}
The responsiveness to external triggers presented in
Theorem~\ref{thm:resp-ext-trg} relies on the proof of
convergence for internal system events and anticipation for external
events, Theorem~\ref{thm:finite-internal-events}. These proof will 
need to be done for each individual SCXML
state-chart as they do not hold a priori.  We present a generic approach
...
\SonComment{We change this list to enumeration}
\begin{enumerate}
...
\end{definition}
\SonComment{Add refence to Unless of Unity}

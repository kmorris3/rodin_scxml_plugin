...
\SonInlineCommented{
For our interpretation of Statecharts in \mbox{\iUMLB}, we used the ``run-to-completion'' semantics of Statecharts.  In particular, we have carefully designed our translated model such that the semantics is captured as a generic abstract model, which is subsequently refined by the translation of the actual SCXML model.  An advantage of this approach is that we can easily adapt replace the basis model with other alternative semantics~\mbox{\cite{Eshuis_2009}} without the changing the translation of the actual SCXML model. 
}
\SonCommented{@Colin: What do you call the translation of the actual SCXML model? (as in constrast to the basis abstract model)}

...
\begin{response}
	\ColinInlineComment{Delete "are contained within an Event-B machine"}
\end{response}
...
\begin{response}
	\ColinInlineComment{Delete  "while Event-B events are expected to already exist to represent the transitions" }
\end{response}
...
\begin{response}
	\ColinInlineComment{Maybe we should not say introduced because here we are talking about the problem not the model. Change to "This safety property should hold at the first refinement and be preserved in all future refinements" }
\end{response}

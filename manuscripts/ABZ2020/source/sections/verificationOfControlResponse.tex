...
Therefore, as well as verifying invariant properties, we would like to verify \SonChange{the liveness of the system, or more}{the system's responsiveness.  More} specifically in this case, \SonAdd{we want to ensure }that the controller responds to external triggers to make appropriate modifications to the system variables. 
These kind of live responses can not be verified by proof of invariants since they \SonChange{involve}{are} temporal properties.
...
\end{center}
\SonChange{Where}{where} the predicate concerns variables |v| that the system maintains, and may refer to old values |old(v)| that existed when the external trigger occurred.

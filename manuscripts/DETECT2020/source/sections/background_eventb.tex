...
Refinement in \EventB is reasoned on an event basis.  A (concrete) event |f|
refines an (abstract) event |e| if whenever |f| is enabled then |e| is
also enabled (guard strengthening), and the action of |f| can be
simulated by |e|.  More information about \EventB refinement can be
found in~\cite{abrial10:_model_event_b}.
\SonCommentTo{Colin}{Can you check if it makes sense.}
